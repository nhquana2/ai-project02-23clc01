...
class ImageManager:
    def __init__(self, cell_size: int):
        ...
        self.scale_factors = {
            ...
            'stench': 0.5, 'agent': 0.5, 'wumpus': 0.5
        }
        self.load_images()
    ...
    def _scale_images(self):
        for key, image in self.images.items():
            scale_factor = self.scale_factors['agent'] if key.startswith(('agent_', 'wumpus_')) else self.scale_factors.get(key, 1.0)
    ...
    def get_centered_position(self, image_key: str, screen_x: int, screen_y: int):
        scale_factor = self.scale_factors['agent'] if image_key.startswith(('agent_', 'wumpus_')) else self.scale_factors.get(image_key, 1.0)
        scaled_size = int(self.cell_size * scale_factor)
